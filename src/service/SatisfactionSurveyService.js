...
export default class SatisfactionSurveyService extends Service {
  ...
  async getAllVoteData(kind) {
    ...
    if (kind === 'site') {
      const queryStr = "SELECT ss.survey_subject, dui.name as do_user_name, dui.mobile as do_user_mobile, dui.card_num as do_user_card_num, sp.satisfaction_level, sp.options, sp.evaluate_time FROM `" + SatisfactionPoll.tableName + "` sp"
      ...
      const queryStr = `SELECT ui.name as survey_name, ui.card_num as survey_card_num, ui.mobile as survey_mobile, dui.name as do_user_name, dui.mobile as do_user_mobile, dui.card_num as do_user_card_num, sp.satisfaction_level, sp.options, sp.evaluate_time FROM \`${SatisfactionPoll.tableName}\` as sp LEFT JOIN \`${SatisfactionSurvey.tableName}\` ss ON ss.id = sp.survey_id`
